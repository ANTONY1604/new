...
if (isset($submit) && $submit == 'create') {
    $fname = $_FILES['college_employee/profilepic']['name'];
    $tr = explode('.', $fname);
    $ext = strtolower(end($tr));
    if ($ext == 'jpg' || $ext == 'png' || $ext == 'jpeg') {
 
        $dir = 'college_employee/profilepic';
        $newfname = $empid . '.' . $ext;
        $profilepic_path = $dir . '/' . $newfname;
        move_uploaded_file($_FILES['college_employee/profilepic']['tmp_name'], $profilepic_path);
